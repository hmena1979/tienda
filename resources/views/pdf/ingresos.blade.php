	...
	<body>
        ...
        <div class="cuadro">
            <table class="tabla">
                <tr>
                    ...
                    <td width="55%">: {{trim($rcompra->cliente->razsoc)}}</td>
                    <td width="13%" class="negrita">N° DOCUMENTO</td>
                    <td width="12%">: {{$rcompra->cliente->numdoc}}</td>
                </tr>
                {{-- <tr>
            ...
            <table class="tabla">
                <tr>
                    <td width='25%'>
                        ...
                        {{date('d-m-Y',strtotime($rcompra->fecha))}}
                    </td>
                    <td width='25%'>
                        {{-- <span class="negrita">VENCIMIENTO : </span>  --}}
                        {{-- {{date('d-m-Y',strtotime($rcompra->vencimiento))}} --}}
                    </td>
                    <td width='25%'><span class="negrita">FORMA PAGO : </span> {{$rcompra->fpago==1?'CONTADO':'CRÉDITO'}}</td>
                    <td width='25%'><span class="negrita">MONEDA : </span> {{$rcompra->moneda == 'PEN' ? 'SOLES' : 'DÓLARES'}}</td>
                </tr>
            </table>
        </div>
        <div class="detalle mtop5">
            <table>
                ...
                <tbody>
                    @foreach($rcompra->detingresos as $det)
                    <tr>
