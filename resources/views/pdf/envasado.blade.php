            ...
            <tr>
                ...
                <td width="40%" valign='top'>
                    <div class="numero text-center">
                        PLANILLA DE ENVASADO {{ $envasado->tipo==2?' CRUDO':'' }}<br>
                        N° {{str_pad($envasado->numero, 6, '0', STR_PAD_LEFT)}}
                    </div>
                </td>
            </tr>
        </table>
        <div class="cuadro">
            <table class="tabla">
                <tr>
                    ...
                    <td width="20%">: {{ date('d-m-Y',strtotime($envasado->fecha)) }}</td>
                    <td width="22%" class="negrita">FECHA DE PRODUCCIÓN</td>
                    <td width="18%">: {{ date('d-m-Y',strtotime($envasado->fproduccion)) }}</td>
                    <td width="8%" class="negrita">LOTE</td>
                    <td width="22%">: {{ $envasado->lote }}</td>
                </tr>
                <tr>
                    <td class="negrita">SUPERVISOR</td>
                    <td colspan="3">: {{ $envasado->supervisor->nombre }}</td>
                    <td class="negrita">TURNO</td>
        ...
        <div class="detalle mtop5">
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="3"></td>
                        <th>{{ number_format($envasado->detenvasados->sum('cantidad')) }}</th>
                        <th>{{ number_format($envasado->detenvasados->sum('total')) }}</th>
                        <td colspan="2"></td>
                    </tr>
                </tbody>
            </table>
            ...
            <table>
                <tr>
                    <td>
                        ...
                        {!! htmlspecialchars_decode(nl2br($envasado->observaciones)) !!}
                    </td>
                </tr>
            </table>
        </div>
        <div class="detalle mtop5">
            <table>
                <tbody>
                    ...
                    <tr>
                        <td class="negrita">
                            @if ($envasado->supervisor_id)
                            {{ $envasado->supervisor->nombre }}
                            @endif
                        </td>
                        <td class="negrita">
                            @if ($envasado->user_id)
                            {{ $users[$envasado->user_id] }}
                            @endif
                        </td>
                    </tr>
                </tbody>
