				...
				<div class="panelprin shadow">
					<div class="headercontent">
						...
						<ul>
							<li>
                                <div class="cita mt-2">
                                    {!! Form::open(['route'=>'admin.ingcamaras.change']) !!}
							...
							@can('admin.ingcamaras.create')
							<li>
								<a href="{{ route('admin.ingcamaras.create') }}">
									Agregar registro
								</a>
							</li>
							@endcan
					...
					<div class="inside">
						<table id= "gridv" class="table table-responsive table-hover">
							...
							<tbody>
								@foreach($ingcamaras as $ingcamara)
								<tr class="
								@if ($ingcamara->estado > 1)
                                negrita
								@endif
								">
									<td>{{ $ingcamara->numero }}</td>
									<td>{{ $ingcamara->lote }}</td>
									<td>{{ $ingcamara->fecha }}</td>
									<td>{{ $ingcamara->fproduccion }}</td>
									<td>{{ $ingcamara->supervisor->nombre }}</td>
									<td>
										<div class="opts">
											@can('admin.ingcamaras.edit')
											<a class="" href="{{ route('admin.ingcamaras.edit',$ingcamara) }}"datatoggle="tooltip" data-placement="top" title="Editar"><i class="fas fa-edit"></i></a>
											@endcan
											@can('admin.ingcamaras.destroy')
											<form action="{{ route('admin.ingcamaras.destroy',$ingcamara) }}" method="POST" class="formulario_eliminars">
												@csrf
												@method('delete')
