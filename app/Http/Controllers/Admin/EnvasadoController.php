...
class EnvasadoController extends Controller
{
    public function __construct()
    {
		...
		$this->middleware('can:admin.envasados.index')->only('index');
		$this->middleware('can:admin.envasados.create')->only('create','store');
		$this->middleware('can:admin.envasados.edit')->only('edit','update');
		$this->middleware('can:admin.envasados.aprobar')->only('aprobar','abrir');
    }
    ...
    public function index($periodo = '000000')
    {
        ...
        $envasados = Envasado::where('tipo',1)
            ->where('empresa_id', session('empresa'))
            ->where('periodo', $periodo)
            ->get();    
        return view('admin.envasados.index',compact('envasados','periodo'));
    }
    ...
    public function create()
    {
        ...
        $tipo = 1;
        return view('admin.envasados.create', compact('lotes','supervisores','tipo'));
    }
    ...
    public function store(Request $request)
    {
    	...
    	if($validator->fails()){
            ...
            return redirect()->route('admin.envasados.edit',$envasado)->with('store', 'Registro agregado');
        }
    }
    ...
    public function edit(Envasado $envasado)
    {
        ...
        return view('admin.envasados.edit', compact('envasado','lotes','supervisores','equipos','trazabilidad'));
    }
    ...
    public function update(Request $request, Envasado $envasado)
    {
    	...
    	if($validator->fails()){
            ...
            return redirect()->route('admin.envasados.index')->with('update', 'Registro Actualizado');
        }
    }
    ...
    public function destroy(Envasado $envasado)
    {
        ...
        $envasado->delete();
        return redirect()->route('admin.envasados.index')->with('destroy', 'Registro Eliminado');
    }

    public function tablaitem(Request $request, Envasado $envasado)
    {
        if ($request->ajax()) {
            return view('admin.envasados.detalle',compact('envasado'));
        }
    }
    ...
    public function aprobar(Envasado $envasado)
    {
        ...
        return redirect()->route('admin.envasados.edit',$envasado)->with('update', 'Planilla de Envasado Aprobada');
    }
    ...
    public function abrir(Envasado $envasado)
    {
        ...
        return redirect()->route('admin.envasados.edit',$envasado)->with('update', 'Puede editar Planilla de Envasado');
    }
}
